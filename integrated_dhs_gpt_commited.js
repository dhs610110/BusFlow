...
async function init(){
  ...
  try{catalog=await api('/api/catalog');updateLiveStations();const h=await api('/api/health');const missing=Object.entries(h.databases).filter(([k,v])=>!v.exists).map(([k])=>k);$('#dataModeBadge_dhs_gpt_commited').textContent=missing.length?'서버 연결 · 일부 자료 미연결':'서버 연결 · 기존 자료 사용';const notes=[];if(missing.length)notes.push('연결되지 않은 기존 DB: '+missing.join(', ')+'. 해당 자료가 필요한 후보는 제외됩니다.');if(h.holiday_calendar_available===false)notes.push('공휴일 패키지가 없어 토·일만 휴일로 구분합니다. 기존 추가 requirements를 설치해주세요.');$('#databaseNotice').textContent=notes.join(' ');}
